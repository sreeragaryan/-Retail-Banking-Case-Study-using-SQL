...
-- Q1.	Create DataBase PROJECT and Write SQL query to create  schema with constraints

create database project;
use project;

-- CREATING TABLE Branch_mstr
create table Branch_mstr
(branch_no int primary key,name char(50) not null);
...
create view cust as
(select c.* ,b.name as branch_city from customer as c 
join branch_mstr as b 
on b.name = c.city_of_residence);
select *from cust;
...
insert into transactions values (1,1,1,1,2000,'deposit',now());

insert into transactions values (2,2,2,2,3000,'withdraw',now());
...
-- server - Data Export - select database - Export to dump project folder - start dump
...
join order_header oh using(order_id))t
